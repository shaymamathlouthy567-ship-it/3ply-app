document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupDial(elementId, min, max, initial, isRolls) {
        ...
        let startAngle = (startPercent * 280) - 140; // Sweeps from -140deg to +140deg
        ...
        function updateRotation(e) {
            ...
            // Adjust so 0 degrees is top center
            angle += 90;
            if (angle < -180) angle += 360;
            if (angle > 180) angle -= 360;

            // Limit the dial from spinning upside down (lock to -140 and 140 degrees)
            if (angle > 140) angle = 140;
            if (angle < -140) angle = -140;

            rotator.style.transform = `rotate(${angle}deg)`;

            // Convert angle back to the number value
    ...
    btn.addEventListener('click', () => {
        // Calculate the actual purchase date based on the slider
